...
* **vision_detector** provides vision-based object detection capabilities. The main algorithms include R-CNN, SSD, and Yolo, which are designed to perform single DNNs for real-time performance. Multiple classes of detection are supported, such as cars and passengers.
* **pixel_tracker** provides a tracking function for the results of **vision_detector**. The algorithm is based on the Beyond Pixels algorithm, and the results of tracking on an image plane are combined with the results of **lidar_detector** through **fusion_tools**. 
...
* **fusion_tools** combines the results from **lidar_detector** and **pixel_tracker**. The class information identified by **vision_detector** is added to the clusters of point cloud detected by **lidar_detector**. 
